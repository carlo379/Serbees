...
- (void) loadImageWithKeys:(NSArray *)keys completion:(void(^__strong)())updateBlock
{
    ...
    CKReference *imageRecordReference = self.postRecord[ServiceImageKey];
    ...
    imageOp.fetchRecordsCompletionBlock = ^(NSDictionary *recordDict, NSError *error) {
        if(error.code == CKErrorPartialFailure) {
            error = error.userInfo[CKPartialErrorsByItemIDKey][[self.postRecord[ServiceImageKey] recordID]];
        }
        ...
        if(errorResponse == AAPLPostErrorSuccess)
        {
            ...
            self.imageRecord = [[ServiceImage alloc] initWithRecord:fetchedImageRecord];
            updateBlock();
        }
