...
@property (strong, atomic) ServicePost *lastPostSeenOnServer;
@property (strong, atomic) NSArray *tagArray;
@property (strong, atomic) NSArray *searchArray;
@property (strong, atomic) NSArray *sectionsArray;
@property (atomic) CGFloat range;
...
- (void) resetWithTagString:(NSString *)tags andSections:(NSArray *)sectionsArray andRange:(NSInteger)range toLocation:(CLLocation *)location
{
    ...
    // Populate Sections into Property
    self.sectionsArray = sectionsArray;
    
    // Populate Range into property
    self.range = range*1000;
    
    // Populate Location into property
    ...
    self.searchArray = [tags isEqualToString:@""] ? [[NSArray alloc] init] : [tags componentsSeparatedByString:@" "];
    self.postCursor = nil;
...
- (void) resetWithTagString:(NSString *)tags forUser:(CKDiscoveredUserInfo *)user{
    ...
    self.isLoadingBatch = NO;
    self.haveOldestPost = NO;
    [self loadUserPostsWithUser:user];
}
...
- (void) loadNewPostsWithAAPLPost:(ServicePost *)post
{
    ...
    if(post) {
        for (NSString *tag in self.tagArray) {
            if(![post.postRecord[ServiceTagsKey] containsObject:tag])
                post = nil;
        }
    }
    ...
    CKQuery *postQuery = [[CKQuery alloc] initWithRecordType:ServiceRecordType predicate:finalPredicate];
    ...
    queryOp.desiredKeys = @[ServiceWebsiteKey,ServiceImageKey,ServiceDescriptionKey,ServiceDescriptionTagsKey,ServiceTagsKey,ServiceNameKey,ServiceNameTagsKey,ServiceContactKey,ServicePhoneKey,ServiceEmailKey,ServiceAddressTagsKey,ServiceAddress1Key,ServiceAddress2Key,ServiceLocationKey,ServiceRecordIDKey];
    ...
    __block ServicePost *lastRecordInOperation = nil;
    queryOp.recordFetchedBlock = ^(CKRecord *record) {
        ...
        NSUInteger matchingRecord = [newPosts indexOfObjectPassingTest:^BOOL(ServicePost *obj, NSUInteger idx, BOOL *stop) {
            if([obj.postRecord.recordID isEqual:record.recordID]) return YES;
            else return NO;
        }];
        if(matchingRecord == NSNotFound)
        {
            ServicePost *fetchedPost = [[ServicePost alloc] initWithRecord:record];
            [newPosts addObject:fetchedPost];
            [fetchedPost loadImageWithKeys:@[ServiceImageFullsizeKey,ServiceImageThumbnailKey] completion:^void(){
    ...
    queryOp.queryCompletionBlock = ^(CKQueryCursor *cursor, NSError *operationError){
        ...
        if(error == AAPLPostManagerErrorSuccess)
        {
            ...
            [newPosts sortUsingComparator:^NSComparisonResult(ServicePost *post1, ServicePost *post2) {
            ...
            for(ServicePost *post in newPosts)
            {
...
- (void)loadBatch
{
    ...
    else
    {
        ...
        if([self.sectionsArray containsObject:@"tags"]){
            for(NSString *tag in self.tagArray)
            {
        ...
        // Create predicate out of SERVICE NAMES. If self.tagArray is empty we should get every post
        if([self.sectionsArray containsObject:@"serviceName"]){
            for(NSString *serviceName in self.tagArray)
            {
                NSPredicate *queryPred = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@"NameTags CONTAINS \'%@\'",serviceName]];
                [subPredicates addObject:queryPred];
            }
        }
        
        // Create predicate out of DESCRIPTION. If self.tagArray is empty we should get every post
        if([self.sectionsArray containsObject:@"description"]){
            for(NSString *description in self.tagArray)
            {
                NSPredicate *queryPred = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@"DescriptionTags CONTAINS \'%@\'",description]];
                [subPredicates addObject:queryPred];
            }
        }
        
        // Create predicate out of ADDRESS1 and ADDRESS2. If self.tagArray is empty we should get every post
        if([self.sectionsArray containsObject:@"address"]){
            for(NSString *address in self.tagArray)
            {
                NSPredicate *queryPred = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@"AddressTags CONTAINS \'%@\'", address]];

                [subPredicates addObject:queryPred];
            }
        }
        // Create predicate out of ADDRESS1 and ADDRESS2. If self.tagArray is empty we should get every post
        if([self.sectionsArray containsObject:@"location"]){
            
            NSPredicate *queryPred = [NSPredicate predicateWithFormat:@"distanceToLocation:fromLocation:(Location, %@) < %f", self.location, self.range];
            [subPredicates addObject:queryPred];

        }
        
        // If our tagArray is empty, create a true predicate (as opposed to a predicate containing "Tags CONTAINS ''"
        NSPredicate *finalPredicate = ([self.tagArray count] == 0 && self.range == 0) ? [NSPredicate predicateWithValue:YES] : [NSCompoundPredicate andPredicateWithSubpredicates:subPredicates];
...
- (void)loadUserPostsWithUser:(CKDiscoveredUserInfo *)user
{
    ...
    {
        // Quickly returns if another loadNextBatch is running or we have the oldest post
        if(self.isLoadingBatch || self.haveOldestPost) {
            // Tell Delegate fihish loading or looking for new posts
            [self.delegate finishLoading];
            return;
        }
        else self.isLoadingBatch = YES;
    ...
    else
    {
        ...
        if([self.tagArray count] > 0){
            for(NSString *tag in self.tagArray)
            {
                NSPredicate *tagPred = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@"NameTags CONTAINS \'%@\'",tag]];
                [subPredicates addObject:tagPred];
            }
        }
        
        NSPredicate *userPred = [NSPredicate predicateWithFormat:@"creatorUserRecordID = %@", user.userRecordID];
        [subPredicates addObject:userPred];
    }
